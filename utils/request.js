...
const request = (url, options) => {
  return new Promise((resolve, reject) => {
    let host = app.globalData.baseUrl
    if(options.data.customHost) { 
      host = options.data.customHost
      delete options.data.customHost
    }
    wx.request({
      url: `${host}${url}`,//获取域名接口地址
